...
..  attention:: This section provides information on hacking a commutator or
    ...
    the Coaxial Commutator :ref:`quick_start_:dry:`shorthand`` Guide and :ref:`user_guide_:dry:`shorthand``.
...
other inputs in addition to the :ref:`JSON commands <remote_control_:dry:`shorthand`>` that the commutator already
accepts. Finally, there are some design considerations at the bottom of the page in case you are considering designing
your own commutator.

..  important:: The RF rotary joint and slip rings used in Open Ephys commutator are specified for millions of cycles
                without failure. If you want to replace these components, make sure you get them from a reputable
                manufacturer to maintain their reliability. We have found that
                `Spinner <https://www.spinner-group.com/en/products/rf-rotary-joints>`__ is a very good option for
                getting high quality rotary joints.
...
For ordering the electronics parts that are mounted onto the PCB, `Digikey <https://www.digikey.com>`_,
`Mouser <https://www.mouser.com>`_, and `Arrow <https://www.arrow.com>`_ are go-to vendors.
...
specific RPCs in the JSON format. Refer to the :ref:`remote_control_:dry:`shorthand`` page of the User Guide for more information on that. Other serial
